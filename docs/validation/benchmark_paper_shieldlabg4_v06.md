# ShieldLab G4 v1.0.0: A Unified Multi-Physics Radiation Shielding Platform Benchmarked Against Reference Standards and Dense-Material Literature
...
Radiation shielding analysis is commonly distributed across multiple tools, with separate programs typically required for photon attenuation, buildup factors, charged-particle stopping powers, and Monte Carlo transport. This fragmentation reduces reproducibility and complicates systematic benchmarking of conventional and emerging shielding materials.

ShieldLab G4 v1.0.0 is an open-source multi-physics platform that couples a Geant4 11.4 multithreaded Monte Carlo engine with an analytical library covering photon attenuation, electron and ion stopping (including heavy ions via the ZBL model), exposure buildup factors, effective atomic number, and fast-neutron removal cross-sections. A multi-tier validation was conducted against seven reference codes (NIST XCOM, Phy-X/PSD, WinXCom, NIST ESTAR/PSTAR/ASTAR, SRIM-2013, MCNP6, FLUKA/PHITS) using 56 photon benchmark points across nine materials from 60 keV to 10 MeV, supplemented by electron, proton, novel-material, and literature-based dense-material benchmarks.

For photon mass attenuation coefficients (μ/ρ), ShieldLab G4 achieves a mean absolute deviation of 0.176% over 55 benchmark points, with 93% of points lying within ±1.5% and 98.2% passing a ±2% criterion after exclusion of a single Bismuth L-edge interpolation artefact. The 26-point extension to 10 MeV across Iron, Lead, Tungsten, and six additional materials shows agreement within 0.01% at the selected NIST tabulation energies. Electron total stopping power agrees with NIST ESTAR to within 1.5%, and proton stopping agrees with NIST PSTAR to within 2.0%. Six novel glass and nanocomposite compositions from recent peer-reviewed publications are reproduced to within 0.30% of published Phy-X/PSD values at the Cs-137 energy. Verified hybrid experiment/simulation studies on granite and heavy-metal oxide glasses, together with Geant4/XCOM studies on dense tellurite systems, report attenuation trends consistent with ShieldLab G4 and extend the validation context to dense and high-Z shielding media beyond the elemental benchmark set. A 16-parameter capability comparison further shows that ShieldLab G4 consolidates the principal analytical functions commonly distributed across NIST XCOM, Phy-X/PSD, WinXCom, and SRIM within a single programmable environment.

These results indicate that ShieldLab G4 provides a coherent and well-benchmarked environment for analytical shielding calculations, with integrated Geant4 capability for geometrically explicit and buildup-dominated problems.
...
Because the late-stage source audit invalidated the original point-by-point reuse of one experimental attenuation dataset, the manuscript no longer imports direct experimental values into a reconstructed benchmark table. Instead, this section retains only verified literature contexts in which experiment, tabulation, and simulation were explicitly compared by the original authors.

The two strongest verified anchors are summarised in Table 8. Both studies concern dense multicomponent shielding systems rather than the standard elemental and simple-compound set used in §4.1, but they remain relevant because they test the same narrow-beam attenuation workflow against external measurements.

**Table 8. Verified experimental and hybrid literature support for dense-material attenuation studies**
...
Taken together, these studies provide external support for the analytical framework used by ShieldLab G4: when material composition is well characterised and the geometry is narrow-beam, experiment, XCOM-derived attenuation quantities, and Monte Carlo transport generally converge within the uncertainty bounds expected for engineering shielding studies in the Compton-dominated regime.
...
11. **Experimental and hybrid literature support.** Verified dense-material studies on granite and heavy-metal oxide glasses [28,29] report consistent agreement patterns among direct measurement, XCOM-derived attenuation data, and MCNP(X)-based transport. These studies provide external support for the analytical and transport framework used by ShieldLab G4 without requiring point-by-point transplantation of heterogeneous published datasets into the present benchmark table.
...
Overall, the results indicate that ShieldLab G4 provides a coherent and well-validated alternative to the combined use of NIST XCOM, Phy-X/PSD, WinXCom, and SRIM for routine analytical shielding work, while also retaining integrated Geant4 capability for geometrically explicit or buildup-dominated problems. The extended analyses in §§4.8–4.14 broaden the assessment beyond scalar agreement metrics to the wider physical parameter space relevant to practical shielding design. Ongoing development is focused on beta-particle dose-rate calculations, expanded neutron interaction support, and automated material-optimisation workflows.
