...
**Manuscript version:** v04 (v03-r4 → v04: Phase 4 internal audit applied — abstract RVE wording, H1 energy qualifier, §4.2 beam description, §5.1.3 EPDL97 citation, §5.2.2 stale note removed, §3.3 feasibility note, §8 limitations updated; figure style guide compliance applied)
...
Nanoparticle-loaded radiation-shielding materials are routinely simulated in Geant4 as elemental homogeneous mixtures, despite the term *nanocomposite* implying a resolved microstructure. The validity boundary of this homogenisation has not been systematically mapped against the explicit-geometry alternatives that Geant4 actually supports, and the computational cost of moving beyond homogenisation is rarely quantified. This paper presents a controlled benchmark of three Geant4 modelling regimes applied to an HDPE/Bi₂O₃ nanocomposite at volume fraction $\phi_f = 0.189$ (68.6 wt% Bi₂O₃, $\rho_\text{eff} = 2.452$ g cm$^{-3}$, derived from the linear mixture rule with pure-phase densities $\rho_f = 8.9$ and $\rho_m = 0.95$ g cm$^{-3}$): (A) effective-medium mass-fraction `G4Material`; (B) explicit nanoparticle placement via `G4PVParameterised` inside a 1 µm representative-volume-element (RVE) cube; and (C) voxelised `G4MultiUnion` inclusion clusters inside a 250 nm RVE ($N = 45$ spheres). All three regimes use the same Geant4 11.4 build, the same physics list (`G4EmLivermorePhysics`), and a matched photon energy grid spanning 30 keV–1.33 MeV. The primary endpoint is the mass attenuation coefficient $\mu/\rho(E)$ compared against NIST XCOM.

Two regime-A (effective-medium) benchmarks are reported: (i) an XCOM validation run at 30 wt% Bi₂O₃ ($\rho = 2.45$ g cm$^{-3}$, $10^6$ histories per point) that yields mean and maximum absolute percent differences of **0.549%** and **2.013%** against XCOM, satisfying pre-registered acceptance thresholds; and (ii) a cross-regime companion run at the composition matching the RVE phases (68.6 wt% Bi₂O₃, $\rho = 2.452$ g cm$^{-3}$, same energy grid), which serves as the regime-A baseline for the H1 test. Regimes B and C (explicit RVE, G4PVParameterised and G4MultiUnion respectively) use random-sequential-addition placement of Bi₂O₃ spheres (radius 25 nm, $\phi_f = 0.189$, 68.6 wt%) inside the RVE host; simulations are executed at $10^7$ histories per energy point. Regime B (1 µm RVE, $N = 2{,}888$ spheres) confirms effective-medium agreement within the 3% H1 gate at 30–150 keV ($\mu_B/\mu_{A,\phi} = 0.975$–0.984), correctly resolves the Bi K-edge jump (×2.3 at 100 keV), and agrees with NIST XCOM to within 1.6–4.0% at those energies. Regime C (250 nm RVE, $N = 45$ spheres, `G4MultiUnion`, distributed beam sampling) confirms H1 at 30–150 keV ($\mu_C/\mu_{A,\phi} = 0.982$–1.016), with XCOM agreement of −2.3% to +1.2% at those energies. The `G4Voxelizer` memory scaling ($(2N)^3$ cells) limits the feasible node count to $N \lesssim 50$ on an 8 GB workstation; this is characterised as a scalability finding. All three regimes are complete and the cross-regime comparison figure (Figure F-cross, A+B+C) has been generated.

**Keywords:** Geant4 · radiation shielding · nanocomposite · effective medium · `G4MultiUnion` · `G4PVParameterised` · representative volume element · mass attenuation coefficient · Monte Carlo benchmarking · reproducible workflow
...
Figure F-cross overlays the regime A $(\mu/\rho)$ data (Table 1), regime B data (Table 2), and regime C data (Table 3) against NIST XCOM on a single log-linear plot with 95% statistical confidence intervals. The figure is generated by

```python
from shieldlab.viz.paper3_figures import generate_regime_comparison_figure
generate_regime_comparison_figure(
    "results/paper3/regime_a_hdpe_bi2o3_phi0p189",  # A(phi0p189): 68.6 wt%, same composition as B and C
    "results/paper3/regime_b_hdpe_bi2o3",
    "results/paper3/regime_c_hdpe_bi2o3",
    output_dir="results/paper3/figures"
)
```

and saved as `results/paper3/figures/paper3_cross_regime_mac.png/.pdf`. **Figure F-cross (all three regimes: A+B+C).** The figure overlays regime A(phi0p189) (Table 1b), regime B (Table 2), and regime C (Table 3) against NIST XCOM. The three curves overlap closely at 30–150 keV, confirming H1 for all three regimes. Above 356 keV, regime B and C points scatter around XCOM due to statistical noise ($N_\text{att} < 600$ for regime B, $N_\text{att} < 130$ for regime C at those energies); error bars on those points are large and the XCOM curve provides the reliable reference.
...
- **Software:** ShieldLab G4 v1.1 (tag to be assigned upon paper submission), MIT licence. Source at the repository root [README.md](README.md).
- **Data:** all benchmark inputs (study JSONs), raw scorer outputs, derived CSVs, and figure-generation scripts are released under CC-BY 4.0 in `results/paper3/`.
...
14. **[To be added — verified Bi₂O₃/HDPE experimental nanocomposite paper, by DOI lookup]**
15. **[To be added — verified WO₃/polymer experimental nanocomposite paper, by DOI lookup]**

---

# Appendix A — Implementation Plan (retained as engineering reference)

This appendix records the engineering work packages that were completed to produce the results in this manuscript.

## A.1 Completed work packages

| WP | Description | Status |
|---|---|---|
| WP1 | Nano-composite material command (regime A++) | Complete — `MaterialRegistry` supports mass-fraction nanocomposite definition |
| WP2 | RVE explicit geometry — `G4PVParameterised` (regime B) | Complete — `NanoParticleParameterisation` class, RSA algorithm, DetectorConstruction integration |
| WP3 | RVE explicit geometry — `G4MultiUnion` (regime C) | Complete — `BuildRVEMultiUnion` with Voxelize() |
| WP4 | Macro writer — RVE energy-grid support | Complete — `macro_writer.py` handles `geometry.type ∈ {rve_explicit, rve_multiunion}` |
| WP5 | Study configs for regime B and C | Complete — `paper3_hdpe_bi2o3_regime_b_rve.json`, `paper3_hdpe_bi2o3_regime_c_multiunion.json` |
| WP6 | Cross-regime figure function | Complete — `paper3_figures.py::generate_regime_comparison_figure()` |
| WP7 | Regime A high-stat benchmark | Complete — passed, results in `results/paper3/regime_a_hdpe_bi2o3_highstat/` |
| WP8 | Regime B benchmark simulation | In progress — simulation running |
| WP9 | Regime C benchmark simulation | Pending — to start after regime B completes |
| WP10 | Manuscript v03 | Complete — this document |

## A.2 Remaining work before submission

- [ ] Fill Tables 2–3 from regime B and C sweep_summary.csv
- [ ] Insert Figure F-cross
- [ ] Verify DOIs for references 13–14
- [ ] Add affiliation and contact details
- [ ] Final proofread and reference formatting per target journal style
- [ ] Zenodo deposit of raw results + code tag
